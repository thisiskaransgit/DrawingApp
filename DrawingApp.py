...
# Configure the alignment , font size and color of the text
        text=Text(root)
        text.tag_configure("tag_name", justify='center', font=('Times New Roman',25),background='#191c42',foreground='orange')
...
# Pick a color for drawing from color pannel
        ...
        i=j=0
        for color in colors:
            Button(self.pick_color,bg=color,bd=2,relief=RIDGE,width=1,command=lambda col=color:self.select_color(col)).grid(row=i,column=j)
            i+=1
            if i==6:
                i=0
                j=1
 ...
 # Erase Button and its properties   
        self.eraser_btn= Button(self.root,text="Eraser",bd=4,bg='white',command=self.eraser,width=7,relief=RIDGE)
        self.eraser_btn.place(x=0,y=197)

# Reset Button to clear the entire screen 
        self.clear_screen= Button(self.root,text="Clear Screen",font=('arial',9),bd=4,bg='white',command= lambda : self.background.delete('all'),width=7,relief=RIDGE)
        self.clear_screen.place(x=0,y=227)

# Save Button for saving the image in local computer
        self.save_btn= Button(self.root,text="ScreenShot",font=('arial',9),bd=4,bg='white',command=self.save_drawing,width=7,relief=RIDGE)
        self.save_btn.place(x=0,y=257)

# Background Button for choosing color of the Canvas
        self.bg_btn= Button(self.root,text="Background",font=('arial',9),bd=4,bg='white',command=self.canvas_color,width=7,relief=RIDGE)
...
#Creating a Scale for pointer and eraser size
        ...
        self.pointer_frame.place(x=0,y=320,height=200,width=90)

        self.pointer_size =Scale(self.pointer_frame,orient=VERTICAL,from_ =48, to =0, length=168)
        self.pointer_size.set(1)
        self.pointer_size.grid(row=0,column=1,padx=20)
